...
typedef struct stream_mem_sync_s
{
    uint64_t 
}
typedef struct stream_mem_sync_t
{
    void *base;
    ucp_mem_h memh;
    stream_rkey_t rkey;
...
typedef struct stream_mem_data_t
{
    void *base;
    ucp_mem_h memh;
    stream_rkey_t rkey;
...
typedef struct _stream_ucx_t
{
    ...
    stream_fn fn;
    stream_mem_data_t buffer_a;
    stream_mem_data_t buffer_b;
    stream_mem_data_t buffer_c;
...
double stream_root_thread(void *arg) {
    ...
    for (j = 0; j < iters; j++)
    {
        ...
        (stream_net->fn)(tmp_acc, b, c, count, thread_ctx);
...
void *stream_peer_thread(void *arg) {
    ...
    for (int j = 0; j < iters; j++)
    {
        ...
        stream_net->thread_sync[thread_ctx->idx].v[PEER] = WAIT;
         ...
         (stream_net->fn)(tmp_acc, &b[count_offset], &c[count_offset],
             count, thread_ctx);
...
static void stream_buffer_free(stream_ucx_t *stream_net, stream_mem_data_t *mem)
{
...
static int exchange_rkeys(stream_ucx_t *stream_net, stream_mem_data_t *buffer)
{
    MPI_Status mpi_status;
    ucs_status_t status;
...
static void stream_register_buffer(stream_ucx_t *stream_net, DTYPE *buffer,
    stream_mem_data_t *mem, size_t size)
{
